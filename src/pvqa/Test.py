...
def demo1():
    ...
    :return:
    """
    model_dirpath = Path('../../Trained_Models/PVQA_ResNet50')
    ...
    features_computer = FeatureExtractor.PvqaFeaturesComputer(backbone_network=train_configs['backbone_network'])
    pvqa_model = PvqaModel.load_model(model_dirpath)

    video = skvideo.io.vread(video_path.as_posix())
    all_features = features_computer.compute_pvqa_features(video)
    score = pvqa_model.predict(all_features).squeeze()
...
def demo2():
    ...
    for video_path in sorted(videos_dirpath.iterdir()):
        ...
        video = skvideo.io.vread(video_path.as_posix())
        pvqa_features = features_computer.compute_pvqa_features(video)
        score = pvqa_model.predict(pvqa_features)
        end_time1 = time.time()
